	...
	return (
		<Fragment>
			<div className="flex flex-col items-center justify-center mx-2 my-8 p-4 rounded-lg shadow w-96 ">
				...
				<ul className="m-4">
					{nations.country.map((nation) => (
						<li key={nation.country_id} className="text-blue-400 font-bold">
							{nation.country_id} - {nation.probability}
						</li>
					))}
				</ul>
			</div>
			...
			<div className="flex flex-col items-center justify-center mx-2 my-8 p-4 rounded-lg shadow w-96 ">
				...
				<h1 className="font-bold m-4 text-blue-400">
					{(gender.gender != null &&
						gender.gender.charAt(0).toUpperCase() + gender.gender.slice(1)) +
						" - " +
						(gender.gender != null && gender.probability)}
				</h1>
			</div>
		</Fragment>
	);
