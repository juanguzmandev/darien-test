...
function App() {
	...
	useEffect(() => {
		alert(
			"Your Possible ID is a tiny and fun app to show your approximated gender, age and nationality based on your name. Have fun!"
		);
	}, []);
		...
		<React.Fragment>
			...
			<section className="flex flex-col items-center justify-center my-4">
				<input
					...
					name="name"
					placeholder="Name"
					className="w-2/4 lg:w-1/4 flex justify-center m-10 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
					onChange={voidNameFilled}
				></input>
				<button
					type="button"
					className="transition duration-700 w-2/4 lg:w-1/4 m-4 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded"
					onClick={() => findResults(name.current.value)}
				>
					Submit
				</button>
				<section className="flex flex-wrap justify-center my-20 w-full">
					{!lockedLoading && loading && !error && !voidName && !voidResults && (
						<Loading />
					)}
					...
					{!loading &&
						...
						!voidResults &&
						name.current.value === "" && (
							<Error text="Name is void. Please, type in your name" />
						)}
					{!loading && error && !voidName && voidResults && (
						<Error text="There's no hits for this name. Let's try another one! 🥪" />
					)}
					...
					{!loading &&
						...
						nations &&
						name.current.value !== "" && (
							<Results age={age} gender={gender} nations={nations} />
						)}
				</section>
			</section>
			<Footer>
